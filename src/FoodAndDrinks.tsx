...
export const initialDrinksData: FoodAndDrink[] = [
    ...
    <div className="gifts-container">
      <header className="gifts-header">
        <div className="gifts-center-content">
          <img src="./images/Food.png" alt="Gifts" className="gifts-icon" />
          <h1 className="gifts-title">FoodAndDrinks</h1>
        </div>
        <button
          type="button"
          ...
          <tbody>
            {fields.map((row, index) => (
              <tr key={row.id}>
                ...
                <td>
                  <CurrencyInputField
                    name={`foodAndDrinks.${index}.budget`}
                    form={form}
                    placeholder={"0.00"}
                ...
                <td>
                  <div className="currency-input-with-status">
                    <CurrencyInputField
                      name={`foodAndDrinks.${index}.spent`}
                      form={form}
                      placeholder={"0.00"}
                      prefix={currencyPrefix}
                    />
                    {+getValues(`foodAndDrinks.${index}.spent`) <=
                    +getValues(`foodAndDrinks.${index}.budget`) ? (
